...
onready var button = load("res://Scenes/DialogSystem/DialogButtonTwo.tscn")
...
#onready var exit_button = $DialogBox/Exit_Button
...
#	exit_button.visible = false
	...
	_ink_player.ink_file = load("res://Stories/Testing/PartOne.ink.json")
...
#	if "PAUSE" in tags:
#		print("Story paused by Ink tag.")
##		exit_button.visible = true
#		return
	var base_speed = 0.05  # seconds per character
	var char_count = text.length()
	var duration = clamp(char_count * base_speed, 0.3, 5)  # min 0.3s, max 3.5s
...
#	if _ink_player.current_path_string() == "SceneTwo":
#		should_continue = false
#		return
#	if should_continue and _ink_player.can_continue():
...
func move_dialog_box(offset: Vector2):
	dialog_box.rect_position += offset
...
#func start_story(path = "start"):
#	visible = true
#	_ink_player.reset()  # Optional: reset if you're starting fresh
