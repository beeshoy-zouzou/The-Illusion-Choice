...
func _process(delta):
	if player_in_range and Input.is_action_just_pressed("interact"):
		door_lock.play()
		DoorPrompt.show_message("You are returning to the previous room.... Entering....")
...
func _on_FirstDoorThirdRoom_body_entered(body):
	if body.is_in_group("player"):
		player_in_range = true
		DoorPrompt.show_prompt("Press Q to enter")


func _on_FirstDoorThirdRoom_body_exited(body):
	if body.is_in_group("player"):
