...
func _process(delta):
	if player_in_range and Input.is_action_just_pressed("interact"):
		DoorPrompt.show_message("You are heading to the mystery room, where you can not return back")
...
func _on_FourthDoorThirdRom_body_entered(body):
	if body.is_in_group("player"):
		player_in_range = true
		DoorPrompt.show_prompt("Press Q to enter")


func _on_FourthDoorThirdRom_body_exited(body):
	if body.is_in_group("player"):
